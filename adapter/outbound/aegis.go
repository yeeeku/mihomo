...
func (a *Aegis) DialContext(ctx context.Context, metadata *C.Metadata, opts ...dialer.Option) (_ C.Conn, err error) {
	return a.DialContextWithDialer(ctx, dialer.NewDialer(a.Base.DialOptions(opts...)...), metadata)
}

// DialContextWithDialer implements C.ProxyAdapter -- TCP dial with chained DialerProxy support.
func (a *Aegis) DialContextWithDialer(ctx context.Context, d C.Dialer, metadata *C.Metadata) (_ C.Conn, err error) {
	if len(a.option.DialerProxy) > 0 {
		d, err = proxydialer.NewByName(a.option.DialerProxy, d)
		if err != nil {
			return nil, err
		}
	}
...
func (a *Aegis) ListenPacketContext(ctx context.Context, metadata *C.Metadata, opts ...dialer.Option) (C.PacketConn, error) {
